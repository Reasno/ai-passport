...
{
    const lv_image_dsc_t *source = icon_source(icon, scale);
...
{
    uint32_t color = icon == UI_PIXEL_ICON_TASK ? KP_GREEN :
                     icon == UI_PIXEL_ICON_GIFT ? KP_YELLOW : 0xF06AA8;
    return ui_pixel_icon_create(parent, icon, x + 4, y + 4, color, 2);
}
