...
esp_err_t nvs_cache_save_model(void)
{
    ...
    for (size_t i = 0; i < model->task_count; i++) { cJSON *o = cJSON_CreateObject(); cJSON_AddStringToObject(o, "task_id", model->tasks[i].id); cJSON_AddStringToObject(o, "display_name", model->tasks[i].name); cJSON_AddNumberToObject(o, "points", model->tasks[i].points); cJSON_AddBoolToObject(o, "completed_today", model->tasks[i].completed_today); cJSON_AddBoolToObject(o, "self_complete", model->tasks[i].self_complete); cJSON_AddBoolToObject(o, "evidence_required", model->tasks[i].evidence_required); cJSON_AddItemToArray(task_array, o); }
