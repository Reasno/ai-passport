...
| Bluetooth LE | 应用固件已禁用；设备间功能使用 ESP-NOW | 独立的 Recovery 分区提供 BLE 安装服务 | 应用代码不得链接 Bluetooth host/controller；保持 Recovery 分区完整 |
...
应用也可以使用 ESP-IDF 提供的定时器、FreeRTOS 任务和内部 Flash/NVS；番茄钟分支提供了 NVS 示例。Wi-Fi 仍是 ESP-IDF 应用服务而非 BSP API。应用固件已禁用 Bluetooth；BLE 安装由独立的 Recovery 分区提供。`demo/claude-buddy-port` 仍是更完整的 BLE 应用架构参考，不能替代对当前板卡天线、射频表现、功耗和共存行为的实测。
...
在主页或游戏列表长按 B3 一至五秒，可为两台设备建立配对。“双人打地鼠”直接复用已有配对关系，游戏页面本身不会触发配对。哥哥设备发起本局，用 B1/B2 上下移动共享准星、B3 开枪；妹妹设备用 B1/B2 左右移动准星、B3 为共享的单发弹夹换弹。20 秒内合作击中 5 只即获胜；每只地鼠即使未命中，也会在 3 秒后自动换格。所有页面继续支持长按 B3 五秒切换隐私模式。
