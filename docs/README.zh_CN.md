...
在主页或游戏列表长按 B3 一至五秒，可为两台设备建立配对。“打地鼠”直接复用已有配对关系，游戏页面本身不会触发配对。哥哥设备发起本局，用 B1/B2 上下移动共享准星、B3 开枪；妹妹设备用 B1/B2 左右移动准星、B3 为共享的单发弹夹换弹。20 秒内合作击中 3 只即获胜；每只地鼠即使未命中，也会在 3 秒后自动换格。所有页面继续支持长按 B3 五秒切换隐私模式。
