...
音频、低功耗工作任务在最后一次共享状态访问之后才发送完成确认，随后挂起，由生命周期所有者删除。停止超时后保留任务句柄和完成信号量，供后续重试；不能让旧任务清空新任务的句柄。Wi-Fi 分别检查 netif 创建、驱动绑定和事件注册结果。不能依赖内部断言退出或忽略任务创建结果的 SDK 辅助函数处理内存不足。
