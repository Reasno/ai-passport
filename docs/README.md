...
| Bluetooth LE | Disabled in the application firmware; pairing and peer features use ESP-NOW | Factory Recovery at `0x700000` owns the BLE install service | Keep the Recovery partition and bootloader hook intact; application code must not link a Bluetooth host/controller |
...
Applications may also use ESP-IDF timers, FreeRTOS tasks, and internal Flash/NVS; the Pomodoro branch contains an NVS example. Wi-Fi remains an ESP-IDF application service rather than a BSP API. Bluetooth is disabled in the application firmware; mini-program installation remains available through the independent factory Recovery partition. `demo/claude-buddy-port` remains a fuller BLE application architecture reference. The current product and firmware baseline uses 8 MB Flash with a 3 MB factory-app partition plus fixed protected identity and permanent-Recovery regions so derivative firmware stays installable through the mini-program.
...
A successful page switch returns `KP_PAGE_OK <NAME>`; an unsupported name returns `KP_PAGE_ERR <REASON>`. The host tool waits 300 ms after the page acknowledgement before requesting the screenshot.
