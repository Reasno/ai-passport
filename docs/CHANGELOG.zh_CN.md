...
- 重新生成内置 UI 字体，覆盖当前全部打地鼠文案字符，包括合作胜利次数中的乘号「×」。

- 修复双机协作“打地鼠”的胜场统计：Host 和 Client 都会累加各自持久化的累计胜场，并在结果页显示。

- 将双机协作“打地鼠”的胜利目标从击中 5 只下调为击中 3 只。

- 新增双机协作“打地鼠”：哥哥设备作为权威 Host，负责上下移动准星和开枪；妹妹设备负责左右移动准星，并为共享的单发弹夹换弹。双端根据 16 字节 ESP-NOW 全量状态显示同一 3×3 画面，并实现输入去重、状态序列过滤、心跳恢复、关键阶段 ACK/重发、对端超时中止，以及双端各自持久化的胜场战绩。
